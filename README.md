...
A condensed read of the whitepaper. It distinguishes activated protocol facts, target architecture, and the standalone Stele web/MCP product boundary; it is not an API reference or proof of current network liveness. As of 2026-07-16, the Stele public preview is live at [stele.monolythium.com](https://stele.monolythium.com) with zero published services. Browser Wallet v0.4.5 is a prerelease. Hosted Stele MCP exposes exactly two OAuth-protected tools, while the isolated local Stele MCP exposes exactly three read/status tools and no transaction tool. Economic writes, transaction signing, and mainnet remain off.
